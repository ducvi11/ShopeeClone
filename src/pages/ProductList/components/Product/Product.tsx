...
import { formatCurrency, fotmatNumberToSocialStyle, generateNameId } from 'src/utils/utils'
    ...
    <Link to={`${path.home}${generateNameId({ name: product.name, id: product._id })}`}>
      <div className='overflow-hidden rounded-sm bg-white shadow transition-transform duration-100 hover:translate-y-[-0.0625rem] hover:shadow-md '>
        <div className='relative w-full pt-[100%]'>
          <img src={product.image} className='absolute left-0 top-0 w-full bg-white object-cover' alt={product.name} />
        </div>
